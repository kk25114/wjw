...
function dw_kw_transform(){ 
    ...
    select 
		...
		c.NATION_CODE as MZ_OLD, --民族,
		c.EDULEVEL_CODE as XL_OLD, --最高学历,
		c.DEGREE_CODE as XW_OLD, --学位,
		c.BYZY_CODE as BYZY_OLD, --毕业专业,
		c.BIRTHDATE as SR, -- 生日,
		c.REGION_CODE as XZQY_OLD, --行政区划,-- 后面重新赋值
		e.org_standard_code as SSDM_OLD, -- 省code
		f.org_standard_code as SQDM_OLD,  --市code
		g.org_standard_code as QXDM_OLD,  -- 区code
		c.ORGNAME as DWMC_OLD, --单位名称
		c.ORG_BELONG_CODE as DWSS_OLD, --单位所属,
		c.ORGPROPERTY_CODE as DWXZ_OLD, --单位性质,
		c.YEAROFWORK as GZNX, --工作年限,
		c.EXAMINEENO as KSH, --考生考试编号,
		c.CARDTYPE_CODE as ZJLX_OLD, -- 考生证件类型,
		c.CARDNO as ZJH, --考生证件号码,
		c.QUALIFY_CODE as XYJSZG_OLD, --现有技术资格,
		c.examinee_type as KSLX_OLD, --考生类型 (0 非军队 1 军队)
		' ' as  JGJB_OLD, --审核机构级别 ，冗余
	...
	from
			candidate c
	left join 
			organization e  on c.ORG_SHENGJI_ID=e.org_id and  c.PROJECT_ID = e.PROJECT_ID
	left join 
		organization f  on c.ORG_SHIJI_ID=f.org_id and  c.PROJECT_ID = f.PROJECT_ID		
	left join 
		organization g  on c.ORG_BMD_ID=g.org_id and  c.PROJECT_ID = g.PROJECT_ID
	where  c.PROJECT_ID  = UPPER('$project_id') "
...
function dw_pf_transform(){ 
    ...
    SELECT
		e.EXAMINEENO KSH,-- 考生号
		sc.TOTAL_POINT KSDFD,--考生实际得分点
		sc.FULL_POINT MDFD,-- 满得分点
		sc.FINAL_SCORE KSDZDF,-- 考生得分
		an.answer_id ANSWER_ID,-- 答案id
		sc.SCOREREMARK QK,--缺考标志
		p.paper_id SJID,-- 试卷id
		p.ZJ_PAPER_ID SJDM,-- 试卷code
		p.PAPERNAME SJNAME,-- 试卷名称
		jb.SUBJECT_STANDARD_CODE JB_OLD,-- 级别
		jb.SUBJECT_NAME JBNAME,-- 级别名称
		zy.SUBJECT_ID ZYID,--专业id
		zy.SUBJECT_CODE ZYCDM,-- 专业长code
		zy.SUBJECT_STANDARD_CODE ZYDM_OLD,-- 专业code
		zy.SUBJECT_NAME ZYNAME,-- 专业名称
		km.SUBJECT_ID KMID,-- 科目id
		km.SUBJECT_CODE KMCDM,-- 科目code
		km.SUBJECT_STANDARD_CODE KMDM_OLD,-- 专业code
		km.SUBJECT_NAME KMNAME,--科目名称 
		decode(e.PASS_STATUS,1,0,2,1,null) TG, --  是否通过 0是一次通过 1 两次通过 null 是不通过 
		...
		jb.SUBJECT_CODE AS JBCDM -- 报考级别长code
	...
	FROM
		...
		subject jb 
	WHERE
		e.CANDIDATE_ID = s.CANDIDATE_ID 
		AND e.PROJECT_ID = s.PROJECT_ID 
		AND e.EXAMINEENO = sc.EXAMINEENO 
		AND sc.SUB_KM_ID = s.SUB_KM_ID 
		AND e.PROJECT_ID = sc.PROJECT_ID 
		AND e.PROJECT_ID = an.PROJECT_ID 
		AND sc.SUB_KM_ID = an.SUB_KM_ID 
		AND e.EXAMINEENO = an.EXAMINEENO 
		AND sc.PAPER_ID = p.PAPER_ID 
		AND e.PROJECT_ID = p.PROJECT_ID 
		AND s.SUB_KM_ID = km.SUBJECT_ID 
		AND km.PARENT_ID = zy.SUBJECT_ID 
		AND zy.PARENT_ID = jb.SUBJECT_ID 
		AND e.PROJECT_ID =UPPER('$project_id') "
    echo '------------抽取一人一卷得分数据表------------'
...
#################
#依据成绩库数据group 维度抽取题库专项数据 01-10没有
#################
...
function dw_getPaper_transform(){ 
    ...
    --password $pf_ocl_pw  \
    -e "create table tmp_paper_t as SELECT DISTINCT  PAPER_ID from  dw_yryf_t"
    ...
    --query "SELECT PAPER_ID from tmp_paper_t where    \$CONDITIONS " \
...
json=$1
for n in kw_ocl_ip kw_ocl_port kw_ocl_db kw_ocl_user kw_ocl_pw  pf_ocl_ip pf_ocl_port pf_ocl_db pf_ocl_user pf_ocl_pw tk_ocl_ip tk_ocl_port tk_ocl_db tk_ocl_user tk_ocl_pw  hive_db ymonth project_id
do
    var_name=$n
